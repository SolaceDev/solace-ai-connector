...
class DummyAppWithCodeConfig(App):
    app_config = {
        ...
        "components": [
            {"name": "code_comp", "component_module": "code_module"}
        ]
    }
...
def test_app_init_merging():
    ...
    assert app_instance.app_info["broker"]["broker_type"] == "code_broker" # From code (not in YAML)
...
def test_create_simplified_flow_config(app_config_override, expected_components):
    ...
    mock_app._create_simplified_flow_config = App._create_simplified_flow_config.__get__(mock_app, App)
    ...
    generated_component_names = [c.get("component_name", c.get("name")) for c in flow_config["components"]]
    ...
    for i, expected in enumerate(expected_components):
        ...
        if expected == "_broker_input":
            ...
            all_subs = [
                sub for comp in merged_app_info["components"] for sub in comp.get("subscriptions", [])
            ]
            assert flow_config["components"][i]["component_config"]["broker_subscriptions"] == all_subs
        elif expected == "_router":
            assert generated_name.endswith("_router")
            assert flow_config["components"][i]["component_module"] == "subscription_router"
            # Check reference passed to router
            assert flow_config["components"][i]["component_config"]["app_components_config_ref"] == merged_app_info["components"]
...
def test_flow_create_component_group_with_class():
    ...
    mock_flow.create_component_group = Flow.create_component_group.__get__(mock_flow, Flow)
...
def mock_router_dependencies():
    ...
    mock_flow.component_groups = [
         [MagicMock(spec=ComponentBase, module_info={"class_name": "BrokerInput"})], # Mock BrokerInput
         [MagicMock(spec=ComponentBase, module_info={"class_name": "SubscriptionRouter"})], # Mock Router itself
         [mock_comp_a], # Group for comp_a
         [mock_comp_b]  # Group for comp_b
    ]

    app_components_config = [
        {"name": "comp_a", "subscriptions": [{"topic": "data/a/>"}, {"topic": "common/data"}]},
    ...
    mock_config = {
        ...
        "component_config": {
            "app_components_config_ref": app_components_config
        }
    }
...
def mock_component_for_get_config():
    ...
    mock_app.get_config = MagicMock(side_effect=lambda key, default=None: app_level_config.get(key, default))

    # Mock ComponentBase instance
    mock_component = MagicMock(spec=ComponentBase)
    mock_component.parent_app = mock_app
    mock_component.get_app = MagicMock(return_value=mock_app) # Ensure get_app returns the mock app

    # Define component-specific and flow-level configs
    mock_component.component_config = {"comp_param": "comp_value", "shared_param": "comp_shared"}
    mock_component.config = {"flow_param": "flow_value", "shared_param": "flow_shared"} # Represents the component's entry in the flow config

    # Bind the real get_config method to the mock instance
    mock_component.get_config = ComponentBase.get_config.__get__(mock_component, ComponentBase)
...
def test_get_config_hierarchy_precedence(mock_component_for_get_config):
    ...
    assert mock_component_for_get_config.get_config("shared_param") == "comp_shared" # Component overrides App and Flow

    # Remove from component_config to test app precedence
    del mock_component_for_get_config.component_config["shared_param"]
    assert mock_component_for_get_config.get_config("shared_param") == "app_shared" # App overrides Flow

    # Remove from app_config to test flow precedence
    mock_component_for_get_config.parent_app.get_config = MagicMock(side_effect=lambda key, default=None: {"app_param": "app_value"}.get(key, default)) # Simulate removal
    assert mock_component_for_get_config.get_config("shared_param") == "flow_shared" # Flow is last resort
