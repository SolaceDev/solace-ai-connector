...
class TestLangChainChatModelBaseInvoke:
    ...
    def test_message_conversion(self, mock_message_fixture):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        LangChainChatModelBase, "invoke_model"
                        ...
                        with patch.object(
                            ...
                            mock_invoke_model.return_value = Result(
                                "Test response", "test-uuid"
                            )

                            # Initialize the component
                            component = LangChainChatModelBase(
                                info=info_base, config={}
                            )
                        ...
                        data = {
                            "messages": [
                                ...
                                {"role": "chat", "content": "Chat message"},
                            ]
                        }
                        result = component.invoke(mock_message_fixture, data)

                        # Check that invoke_model was called with the correct messages
                        mock_invoke_model.assert_called_once()
                        args = mock_invoke_model.call_args[0]
                        assert args[0] == mock_message_fixture
                        assert isinstance(args[1][0], SystemMessage)
                        assert args[1][0].content == "You are a helpful assistant"
                        assert isinstance(args[1][1], HumanMessage)
                        assert args[1][1].content == "Hello"
                        assert isinstance(args[1][2], AIMessage)
                        assert args[1][2].content == "Hi there"
                        assert isinstance(args[1][3], FunctionMessage)
                        assert args[1][3].content == "Function result"
                        assert isinstance(args[1][4], ChatMessage)
                        assert args[1][4].content == "Chat message"
