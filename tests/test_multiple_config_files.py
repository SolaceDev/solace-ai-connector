...
""")
    ...
    try:
        ...
        assert config["apps"][0]["name"] == os.path.splitext(os.path.basename(filename))[0]
...
""")
    ...
    try:
        ...
        connector = SolaceAiConnector(merged_config, config_filenames=[filename1, filename2])
          ...
          - component_name: user_processor
            ...
            component_processing:
              invoke:
                ...
                params:
                  positional:
                    - evaluate_expression(self:get_config('shared_config_value'))
""")
        f.flush()
        filename = f.name
