...
class RequestResponseFlowController:
    def __init__(self, config: Dict[str, Any], connector):
        ...
        self.streaming_complete_expression = config.get("streaming_complete_expression")
    ...
    def send_message(self, message: Message, data: Any):
        ...
        new_message = Message(
            payload=data, topic=topic, user_properties=user_properties
        )
        ...
        while True:
            ...
            if self.streaming:
                    ...
                    try:
                        ...
                        if event.event_type == EventType.MESSAGE:
                            ...
                            last_message = message.get_data(
                                self.streaming_complete_expression
                            )
                            yield message, message.get_previous(), last_message
                            if last_message:
                                return
                    except queue.Empty:
                ...
                try:
                    ...
                    if event.event_type == EventType.MESSAGE:
                        message = event.data
                        yield message, message.get_previous(), True
                        return
                except queue.Empty:
                    if (time.time() - self.enqueue_time) > self.timeout_s:
                        raise TimeoutError("Timeout waiting for response")
