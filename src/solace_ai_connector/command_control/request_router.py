...
class RequestRouter:
    ...
    def route_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        handler_info, path_params, entity_id = (
            self.entity_registry.find_endpoint_handler(endpoint, method)
        )

        if not handler_info:
            return self._create_error_response(
                request_id, 404, f"No handler found for {method} {endpoint}"
            )
        ...
        if not handler_func or not callable(handler_func):
            return self._create_error_response(
                request_id, 500, f"Invalid handler for {method} {endpoint}"
            )
        ...
        if validation_result:
            return self._create_error_response(
                request_id, 400, f"Parameter validation failed: {validation_result}"
            )
        ...
        try:
            ...
            if not entity:
                return self._create_error_response(
                    request_id, 500, f"Entity {entity_id} not found"
                )
            ...
            log.error(
                "Error calling handler for %s %s: %s",
                method,
                endpoint,
                str(e),
                exc_info=True,
            ...
            return self._create_error_response(
                request_id, 500, f"Error processing request: {str(e)}"
            )
    ...
    def _create_response(
        self, request_id: str, status_code: int, status_message: str, body: Any
    ) -> Dict[str, Any]:
    ...
    def _create_error_response(
        ...
        return self._create_response(
            request_id, status_code, message, {"error": message}
        )
