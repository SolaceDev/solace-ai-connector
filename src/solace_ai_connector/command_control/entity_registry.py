...
class EntityRegistry:
        ...
        try:
            ...
            for endpoint in endpoints:
                try:
                    ...
                    # and return False
                    log.error(
                        "Error registering endpoint for entity %s: %s",
                        entity_id,
                        str(e),
                    )
    ...
    def _register_endpoint(self, entity_id: str, endpoint: Dict[str, Any]) -> None:
        ...
        log.debug(
            "Registered endpoint %s for entity %s with methods %s",
            path,
            entity_id,
            list(methods.keys()),
        )
    ...
    def find_endpoint_handler(
        self, path: str, method: str
    ) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, str]], Optional[str]]:
        ...
        for pattern_str, pattern in self.endpoint_patterns.items():
            ...
            if match:
                ...
                log.warning(
                    "Method %s not supported for endpoint %s", method, path_template
                )
    ...
    def deregister_entity(self, entity_id: str) -> bool:
        ...
        log.info(
            "Entity %s deregistered with %d endpoints",
            entity_id,
            len(patterns_to_remove),
        )
        return True
