...
class CopyListItemTransform(TransformBase):
    def __init__(self, transform_config, index):
        ...
        if self.source_property is None:
            raise ValueError("In CopyListItemTransform, source property not provided in transform configuration")
    ...
    def invoke(self, message, calling_object=None):
        ...
        source_list = message.get_data(self.source_expression, calling_object=calling_object)

        if not isinstance(source_list, list):
            if allow_missing_source:
                return message
            raise ValueError("In CopyListItemTransform, source expression does not resolve to a list")

        # Get the destination list
        dest_list = message.get_data(self.dest_expression, calling_object=calling_object)
        ...
        if not isinstance(dest_list, list):
            raise ValueError("In CopyListItemTransform, destination expression does not resolve to a list")

        # Copy the item from the source list to the destination list
        for index, item in enumerate(source_list):
            if not isinstance(item, dict):
                raise ValueError("In CopyListItemTransform, source list item is not a dictionary")
            ...
            if self.dest_property is not None:
                ...
                if not isinstance(dest_list[index], dict):
                    raise ValueError("In CopyListItemTransform, dest list item is not a dictionary")
